...
public class GlobalExceptionHandler {
    ...
    /**
     * 聊天业务异常（如首条限制 WAIT_REPLY、无权限管理群）：返回原始文案给前端 toast
     */
    @ExceptionHandler
    public Result<Void> handleChatReject(ChatRejectException ex) {
        log.info("聊天业务拒绝：code={}, msg={}", ex.getCode(), ex.getMessage());
    ...
    public Result<Void> handleMissingParam(MissingServletRequestParameterException ex) {
        log.error("缺少必填参数：{}", ex.getParameterName());
